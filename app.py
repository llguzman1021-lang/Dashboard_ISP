...
# ── FIX: Inicialización segura de sesión ──
for _k, _v in [
...
def get_clientes_cmdb(zona: str, equipo: str) -> int:
    ...
    for (z, e), clientes in cmdb.items():
        if z in zona.lower() and e == equipo.lower(): return clientes
    return 0

# =====================================================================
# CARGA Y ENRIQUECIMIENTO (Totalmente BLINDADO)
# =====================================================================
@st.cache_data(ttl=60, show_spinner=False)
def load_data_rango(fecha_ini: date, fecha_fin: date, include_deleted: bool = False,
                    zona_filtro: str = "Todas", serv_filtro: str = "Todos", seg_filtro: str = "Todos") -> pd.DataFrame:
    
    # FORZAR FORMATO ISO PARA EVITAR CONFLICTOS DE POSTGRESQL
    s_date_str = SV_TZ.localize(datetime.combine(fecha_ini, datetime_time(0, 0, 0))).isoformat()
    e_date_str = SV_TZ.localize(datetime.combine(fecha_fin, datetime_time(23, 59, 59))).isoformat()
    ...
    try:
        with engine.connect() as conn:
            return pd.read_sql(text(q), conn, params={"s": s_date_str, "e": e_date_str})
    except Exception as e:
        # Ya no hay fallos silenciosos, si falla lo veremos en pantalla
        st.error(f"Error recuperando base de datos: {e}")
        return pd.DataFrame()

# 🚫 SIN CACHÉ AQUÍ: Evita el problema del dataframe vacío o columnas perdidas
def enriquecer(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _severidad(r) -> str:
        if r['estado'] == 'Abierto':                                       return '🚨 CRÍTICA (En Curso)'
    ...
    df['zona_completa'] = df.apply(lambda r: f"{r['zona']} (General)" if str(r.get('subzona', 'General')).strip() == 'General' else f"{r['zona']} - {r['subzona']}", axis=1)
    return df
...
def calc_kpis(df: pd.DataFrame, fecha_ini: date, fecha_fin: date) -> dict:
    ...
    if not df_exact.empty:
        df_sla_g = df_exact[df_exact['causa_raiz'] != 'Mantenimiento Programado']
        s_cl  = df_sla_g['inicio_incidente'].clip(lower=rng_s)
        e_cl  = df_sla_g['fin_incidente'].clip(upper=rng_e)
        valid = s_cl <= e_cl
...
def dibujar_graficos(df_m):
    ...
    fig_map = px.scatter_mapbox(agg, lat="lat", lon="lon", hover_name="zona_completa", size="Clientes_sz", color="Horas",
                                color_continuous_scale="Inferno", zoom=8.5, mapbox_style="carto-darkmatter",
...
def generar_pdf(label_periodo: str, kpis: dict, df: pd.DataFrame) -> bytes:
    ...
    if not df.empty and 'duracion_horas' in df.columns:
        ...
        if not df_ext.empty:
            ...
            tz.setStyle(tbl_style(rl_colors.HexColor(COLOR_TEAL)))
...
with st.sidebar:
    ...
    if st.button("🚪 Cerrar Sesión", use_container_width=True):
        log_audit("LOGOUT", "Sesión cerrada."); st.session_state.clear(); st.rerun()
    st.markdown("""
...
with tabs[t_idx]:
    ...
    if df_m.empty:
        ...
        fig_sla = px.bar(df_sla_geo, x="SLA (%)", y="Zona", orientation='h', text_auto='.3f', color="SLA (%)",
                         color_continuous_scale=["#ff2b2b","#ff9f43","#29b09d"], title="Ranking de Estabilidad Geográfica")
        ...
        if kpis['t1']['fallas'] == 0:
            ...
            t1b.metric("MTTR Real (Resolución)",  f"{kpis['t1']['mttr']:.2f} horas",  delta=_delta('t1','mttr',    fmt="{:+.2f}", suffix="h"), delta_color="inverse")
            t1c.metric("ACD Real (Afectación)",   f"{kpis['t1']['acd']:.2f} horas",   delta=_delta('t1','acd',     fmt="{:+.2f}", suffix="h"), delta_color="inverse")
    ...
    with tabs[t_idx]:
        ...
        with cf:
            with st.container(border=True):
                ...
                if hi_on and (not es_abierto) and st.session_state.get(f"hf_on_{fk}", False):
                    try:
                ...
                if st.button("💾 Guardar Registro", type="primary"):
                    err = False
                    try:
                        if (not es_abierto) and hi_on and st.session_state.get(f"hf_on_{fk}") and (fi > ff or (fi == ff and hi_val >= hf_val)):
                    ...
                    if not err:
                        with st.spinner("Guardando en Base de Datos…"):
                            try:
                                ...
                                idi_tz = SV_TZ.localize(datetime.combine(fi, hi_db))
                                idi_str = idi_tz.isoformat()
                                
                                if es_abierto or not st.session_state.get(f"hf_on_{fk}"):
                                    idf_str = None
                                else:
                                    idf_tz = SV_TZ.localize(datetime.combine(ff, hf_val))
                                    idf_str = idf_tz.isoformat()
                                ...
                                with engine.begin() as conn:
                                    conn.execute(text("""
                                        INSERT INTO incidents
                                            (zona,subzona,afectacion_general,servicio,categoria,equipo_afectado,estado,
                                             inicio_incidente,fin_incidente,clientes_afectados,impacto_porcentaje,
                                    ...
                                    """), {"z": z_f, "sz": sz_db, "ag": ag, "s": srv_f, "c": cat_f, "e": eq_f,
                                           "est": estado_db, "idi": idi_str, "idf": idf_str, "cl": cl_f, "imp": imp_pct,
...
if role in ('admin', 'auditor'):
    with tabs[t_idx]:
            ...
            with st.expander("Cerrar Falla en Curso (Ticket Abierto)", expanded=True):
                with st.form("form_cerrar_ticket"):
                    ...
                    if st.form_submit_button("Cerrar Ticket", type="primary"):
                        ...
                        fin_val = SV_TZ.localize(datetime.combine(f_fin, h_fin))
                        if fin_val < ini_dt:
                            st.error("❌ La fecha/hora de cierre no puede ser menor a la de inicio.")
                        else:
                            dur_h = max(0.01, round((fin_val - ini_dt).total_seconds() / 3600, 2))
                            with engine.begin() as conn:
                                conn.execute(text("UPDATE incidents SET estado='Cerrado',fin_incidente=:f,duracion_horas=:d,conocimiento_tiempos='Total' WHERE id=:id"),
                                             {"f": fin_val.isoformat(), "d": dur_h, "id": sel_t})
                            log_audit("CLOSE TICKET", f"Ticket ID {sel_t} cerrado.")
                            load_data_rango.clear()
                            st.session_state.flash_msg = "✅ Ticket cerrado correctamente."; st.rerun()

        st.divider()
        st.markdown("#### Edición Avanzada (Tabla Masiva)")
        st.info("⚠️ **Importante:** Haz clic en 'Guardar Ediciones Manuales' antes de cambiar de página.")
        ...
        if df_audit.empty: st.info("No hay datos en el servidor para el periodo.")
        else:
            ...
            ed_df = st.data_editor(
                ...
                column_config={"Sel": st.column_config.CheckboxColumn("✔", default=False), "id": None,
            ...
            ref_df = df_page.drop(columns=drop_cols + (['Sel'] if 'Sel' not in drop_cols else []), errors='ignore').reset_index(drop=True)
            ...
            h_cam = not ref_df.copy().apply(strip_tz).equals(ed_df.drop(columns=['Sel']).copy().apply(strip_tz))
            ...
            if not f_sel.empty:
                if papelera:
                    ...
                    if cb1.button("🗑️ Eliminar Seleccionados", type="primary", use_container_width=True):
                        ...
                        st.session_state.flash_msg = "🗑️ Registros eliminados exitosamente."; st.rerun()

            if h_cam and cb2.button("💾 Guardar Ediciones Manuales", type="primary", use_container_width=True):
                fechas_ok = all(not pd.isnull(r.get('inicio_incidente')) for i, r in ed_df.iterrows()
                        ...
                        for i, r in ed_df.iterrows():
                            if not strip_tz(ref_df.iloc[i]).equals(strip_tz(r.drop('Sel'))):
                                try:
                                    ...
                                    if ini_dt.tzinfo is None: ini_dt = SV_TZ.localize(ini_dt)
                                    ini_str = ini_dt.isoformat()
                                    
                                    if pd.isnull(r.get('fin_incidente')):
                                        fin_str = None; dur_u = 0; con_u = "Parcial"; est_u = "Abierto"
                                    else:
                                        fin_dt = pd.to_datetime(r.get('fin_incidente'))
                                        if fin_dt.tzinfo is None: fin_dt = SV_TZ.localize(fin_dt)
                                        fin_str = fin_dt.isoformat()
                                        dur_u = max(0.01, round((fin_dt - ini_dt).total_seconds() / 3600, 2))
                                        con_u = "Total"; est_u = "Cerrado"
                                except:
                                    fin_str = None; dur_u = 0; con_u = "Parcial"; est_u = "Abierto"
                                    ini_str = SV_TZ.localize(pd.to_datetime(r.get('inicio_incidente'))).isoformat()
                                
                                conn.execute(text("""
                                    UPDATE incidents SET zona=:z,subzona=:sz,afectacion_general=:ag,servicio=:s,categoria=:c,equipo_afectado=:e,
                                        estado=:est,inicio_incidente=:idi,fin_incidente=:idf,clientes_afectados=:cl,causa_raiz=:cr,
                                        descripcion=:d,duracion_horas=:dur,conocimiento_tiempos=:con WHERE id=:id
                                """), {"z": r.get('zona',''), "sz": r.get('subzona',''), "ag": bool(r.get('afectacion_general',True)),
                                      "s": r.get('servicio',''), "c": r.get('categoria',''), "e": r.get('equipo_afectado',''),
                                      "est": est_u, "idi": ini_str, "idf": fin_str, "cl": int(r.get('clientes_afectados',0)),
                                      "cr": r.get('causa_raiz',''), "d": r.get('descripcion',''), "dur": dur_u, "con": con_u, "id": int(r['id'])})
                    log_audit("UPDATE", "Edición masiva."); load_data_rango.clear()
                    st.session_state.flash_msg = "💾 Cambios guardados correctamente."; st.rerun()

            st.divider()
            st.download_button("📥 Descargar CSV de esta Tabla", df_d.drop(columns=drop_cols, errors='ignore').to_csv(index=False).encode(),
                               f"NOC_Export_{fecha_ini}_{fecha_fin}.csv", "text/csv", use_container_width=True)
    t_idx += 1
            ...
            with st.form("form_add_zona", clear_on_submit=True):
                ...
                if st.form_submit_button("Agregar Zona") and nz:
                    try:
                        ...
                        clear_catalog_cache(); st.session_state.flash_msg = "✅ Zona agregada exitosamente."; st.rerun()
                    except: st.toast("Error: Zona duplicada.", icon="❌")
            ...
            with st.form("form_add_eq", clear_on_submit=True):
                ...
                if st.form_submit_button("Agregar Equipo") and ne:
                    try:
                        ...
                        clear_catalog_cache(); st.session_state.flash_msg = "✅ Equipo agregado exitosamente."; st.rerun()
            ...
            with st.form("form_add_causa", clear_on_submit=True):
                ...
                if st.form_submit_button("Agregar Causa") and nc:
                    try:
                        ...
                        clear_catalog_cache(); st.session_state.flash_msg = "✅ Causa agregada exitosamente."; st.rerun()
                ...
                with st.form("form_u", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Crear Cuenta") and nu and np_u:
                        try:
                            ...
                            st.session_state.flash_msg = "✅ Usuario creado exitosamente."; st.rerun()
                        except Exception as e:
                            if "unique" in str(e).lower() or "duplicate" in str(e).lower(): st.toast("❌ Error: Ese nombre de usuario ya existe.", icon="❌")
                            else: st.toast(f"❌ Error: {str(e)}", icon="❌")
                    ...
                    if st.form_submit_button("Restablecer") and u_reset and p_reset:
                        try:
                            with engine.begin() as conn:
                                ...
                                if res.rowcount > 0: st.session_state.flash_msg = "✅ Contraseña actualizada."; st.rerun()
            ...
            with clg:
                try:
                    ...
                    ed_usrs = st.data_editor(df_usrs,
                        column_config={"Sel": st.column_config.CheckboxColumn("✔", default=False), "id": None,
                                       "username": "Usuario", "role": "Rol", "is_banned": "Baneado", "failed_attempts": "Intentos Fallidos"},
                        use_container_width=True, hide_index=True)
                    ...
                    if hay_cambios and u2c.button("💾 Guardar Permisos", type="primary", use_container_width=True):
                        ...
                        st.session_state.flash_msg = "💾 Permisos de usuario guardados."; st.rerun()
