...
DEFAULT_CAUSAS = (
    ("Corte de Fibra por Terceros", True), ("Corte de Fibra (No Especificado)", False), ("Caida de Arboles sobre Fibra", True),
    ("Falla de Energia Comercial", True), ("Corrosion en Equipos", False), ("Danos por Fauna", True),
    ("Falla de Hardware", False), ("Falla de Configuracion", False), ("Falla de Redundancia", False),
    ("Saturacion de Trafico", False), ("Saturacion en Servidor", False), ("Mantenimiento Programado", False),
    ("Vandalismo o Hurto", True), ("Condiciones Climaticas", True),
)
...
# ── Inicializacion SEGURA y COMPLETA de sesion ──
sesion_keys = {
...
if st.session_state.get('flash_msg'):
    if st.session_state.get('flash_type') == 'error':
        st.error(st.session_state.get('flash_msg'), icon="X")
    else:
        st.toast(st.session_state.get('flash_msg'), icon="V")
...
# FIX BUG 7: Restaurado @st.cache_data
@st.cache_data(ttl=60, show_spinner=False)
def enriquecer(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # FIX BUG 6: tz_convert sobre columna completa (no .loc parcial que falla en pandas moderno)
    ...
    # FIX BUG 5: usar df['col'] directamente, no df.get('col', escalar)
    df['duracion_horas']     = pd.to_numeric(df['duracion_horas']     if 'duracion_horas'     in df.columns else 0, errors='coerce').fillna(0.0)
    df['clientes_afectados'] = pd.to_numeric(df['clientes_afectados'] if 'clientes_afectados' in df.columns else 0, errors='coerce').fillna(0).astype(int)

    causas_ext_map = get_causas_con_flag()

    def _severidad(r) -> str:
        # r es Series-row de apply(axis=1) -> .get() ES valido aqui
        if r.get('estado') == 'Abierto':                                              return 'CRITICA (En Curso)'
        if r.get('categoria') == CAT_INTERNA:                                         return 'P4 (Interna)'
        if r.get('duracion_horas', 0) >= 12 or r.get('clientes_afectados', 0) >= 1000: return 'P1 (Critica)'
        if r.get('duracion_horas', 0) >= 4  or r.get('clientes_afectados', 0) >= 300:  return 'P2 (Alta)'
        return 'P3 (Media)'

    df['Severidad']  = df.apply(_severidad, axis=1)
    # FIX BUG 3/5: df['col'] en lugar de df.get('col') para columnas de DataFrame
    ...
    df['zona_completa'] = df.apply(
        lambda r: f"{r.get('zona')} (General)" if r.get('afectacion_general', True)
                  else f"{r.get('zona')} - {r.get('subzona')}", axis=1)
    return df
...
def calc_kpis(df: pd.DataFrame, fecha_ini: date, fecha_fin: date) -> dict:
    ...
    # FIX BUG 2: Usar df['columna'] para filtrado booleano en DataFrame
    # df.get('col') devuelve None si no existe -> AttributeError en operaciones siguientes
    ...
    base["global"]["p1"]           = int((df_ext['Severidad'].str.contains('P1', na=False)).sum())
...
def dibujar_graficos(df_m: pd.DataFrame):
    ...
    df_m = df_m.copy()  # FIX BUG 3: No mutar el DF cacheado original

    st.markdown("### Analisis Geografico y Temporal")
    zonas_coords = {z[0]: (z[1], z[2]) for z in get_zonas()}
    df_map = df_m.copy()
    # FIX BUG 3: df['col'] en lugar de df.get('col') para columnas de DataFrame
    ...
    agg = (df_map.groupby(['zona_completa','lat','lon'])
           ...
           .reset_index())
    agg['Clientes_sz'] = agg['Clientes'].clip(lower=1)  # FIX: agg['Clientes'] no agg.get('Clientes')
    ...
    fig_map = px.scatter_mapbox(agg, lat="lat", lon="lon", hover_name="zona_completa",
        ...
        zoom=8.5, mapbox_style="carto-darkmatter", labels={"Clientes_sz": "Clientes"},
        title="Impacto Geografico por Nodo")
    ...
    # FIX BUG 1: CORREGIDO el SyntaxError - df_m['inicio_incidente'] en lugar de df_m.get('inicio_incidente']
    df_heat = df_m[df_m['inicio_incidente'].notnull()].copy()

    if not df_heat.empty:
        df_heat['Dia']  = pd.Categorical(df_heat['inicio_incidente'].dt.dayofweek.map(dias_map), categories=dias_orden, ordered=True)
        df_heat['Hora'] = df_heat['inicio_incidente'].dt.hour
        df_hm = df_heat.groupby(['Dia','Hora'], observed=False).size().reset_index(name='Fallas')
        fig_hm = px.density_heatmap(df_hm, x='Hora', y='Dia', z='Fallas',
            color_continuous_scale='Blues', nbinsx=24, title="Mapa de Calor (Fallas por Hora)")
    ...
    with c_bar:
        dc = (df_m.groupby('causa_raiz').size().reset_index(name='Alertas')
              .sort_values('Alertas', ascending=True).tail(8))
        fig_b = px.bar(dc, x='Alertas', y='causa_raiz', orientation='h',
            color_discrete_sequence=[COLOR_PRIMARY], text_auto='.0f', title="Top Causas Raiz")
        fig_b.update_traces(textposition='outside')
        fig_b.update_layout(margin=dict(l=0,r=0,t=40,b=0), paper_bgcolor="rgba(0,0,0,0)",
    ...
    try:
        with engine.begin() as conn:
            conn.execute(text("INSERT INTO audit_logs (timestamp,username,action,details) VALUES (:t,:u,:a,:d)"),
                {"t": datetime.now(SV_TZ).replace(tzinfo=None), "u": st.session_state.get("username","Sistema"), "a": action, "d": detail})
    except: pass
...
def generar_pdf(label_periodo: str, kpis: dict, df: pd.DataFrame) -> bytes:
    ...
    def tbl_style(hdr_color):
        return TableStyle([
            ('BACKGROUND',(0,0),(-1,0),hdr_color),('TEXTCOLOR',(0,0),(-1,0),rl_colors.white),
            ('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('FONTSIZE',(0,0),(-1,0),9),
            ('ROWBACKGROUNDS',(0,1),(-1,-1),[rl_colors.HexColor('#f5f5f5'),rl_colors.white]),
            ('FONTNAME',(0,1),(-1,-1),'Helvetica'),('FONTSIZE',(0,1),(-1,-1),9),
            ('VALIGN',(0,0),(-1,-1),'MIDDLE'),('GRID',(0,0),(-1,-1),0.4,rl_colors.HexColor('#dddddd')),
            ('ROWHEIGHT',(0,0),(-1,-1),20),('LEFTPADDING',(0,0),(-1,-1),7),('RIGHTPADDING',(0,0),(-1,-1),7),
    ...
    story += [Paragraph("MULTINET", s_title), Paragraph("Reporte Ejecutivo - Network Operations Center", s_sub),
              ...
              Paragraph(f"<b>Generado:</b> {now_str} (hora El Salvador)", s_body), Spacer(1, 0.5*cm)]

    story.append(Paragraph("1. Metricas Operativas Principales", s_sec))
    kpi_rows = [
        ['Indicador', 'Valor', 'Descripcion'],
        ['SLA Global', f"{kpis['global']['sla']:.3f}%", 'Intervalos fusionados (Excluye mantenimientos)'],
        ['MTTR Real', f"{kpis['t1']['mttr']:.2f} horas", 'Resolucion promedio (clientes > 0)'],
        ['ACD Real', f"{kpis['t1']['acd']:.2f} horas", 'Afectacion promedio percibida'],
        ['Impacto Acumulado', f"{(kpis['global']['db']/24):.2f} dias", 'Horas totales caidas / 24'],
        ['Clientes Impactados', f"{kpis['t1']['clientes']:,}", 'Usuarios afectados (T1)'],
        ['MTBF', f"{kpis['global']['mtbf']:.1f} horas", 'Tiempo medio entre fallas'],
        ['Fallas P1 Criticas', f"{kpis['global']['p1']}", '>12 h o >1000 clientes'],
    ]
    ...
    story.append(Paragraph("2. Salud de Datos", s_sec))
    pend_rows = [['Estado','Cantidad'], ['Fallas Abiertas', str(kpis['abiertos'])],
                 ['Fallas Sin Hora Exacta', str(kpis['t3']['fallas'])], ['Internas', str(kpis['int']['fallas'])]]
    tp = Table(pend_rows, colWidths=[10*cm, 4*cm])
    tp.setStyle(tbl_style(rl_colors.HexColor(COLOR_TEAL)))
    story += [tp, Spacer(1, 0.4*cm)]

    # FIX BUG 4: df['col'] en lugar de df.get('col') en generar_pdf
    if not df.empty and 'duracion_horas' in df.columns:
        df_ext = df[(df['categoria'] != CAT_INTERNA) & (df['estado'] == 'Cerrado')]
        if not df_ext.empty:
            story.append(Paragraph("3. Zonas con Mayor Afectacion", s_sec))
            top_z = df_ext.groupby('zona_completa')['duracion_horas'].sum().nlargest(8).reset_index()
            z_rows = [['Zona / Nodo', 'Horas', 'Dias Equiv.']]
...
def do_login():
    ...
    try:
        ...
        st.session_state.log_err = "Error de conexion DB."
...
if not st.session_state.logged_in:
    ...
    with col_c:
        with st.container(border=True):
            ...
            st.text_input("Contrasena", key="log_p", type="password")
            st.button("Iniciar Sesion", type="primary", on_click=do_login, use_container_width=True)
...
# =====================================================================
with st.sidebar:
    st.caption(f"Usuario: {st.session_state.get('username','?')} ({st.session_state.get('role','viewer').capitalize()}) | NOC v1.0")
    st.divider()
    ...
    a_sel = st.selectbox("Ano",  anios_list, index=idx_anio)
    ...
    st.divider()
    z_sel   = st.selectbox("Zona",     ["Todas"]  + [z[0] for z in get_zonas()])
    ...
    if not df_m.empty:
        st.download_button("Descargar Reporte PDF",
            ...
            file_name=f"Reporte_NOC_{m_sel}_{a_sel}.pdf", mime="application/pdf", use_container_width=True)
    st.divider()
    if st.button("Cerrar Sesion", use_container_width=True):
        log_audit("LOGOUT","Sesion cerrada."); st.session_state.clear(); st.rerun()
    st.markdown("<div style='margin-top:50px;text-align:center;color:#666;font-size:11px;'>Engineered by<br><b>Luis Salvador Guzman Lopez</b></div>", unsafe_allow_html=True)
...
t_idx = 0
if   role == 'admin':   tab_labels = ["Dashboard", "Registrar Evento", "Historial y Edicion", "Configuracion"]
elif role == 'auditor': tab_labels = ["Dashboard", "Registrar Evento", "Historial y Edicion"]
...
with tabs[t_idx]:
    ...
    if df_m.empty:
        st.success("Sin incidentes registrados. La red esta al 100%!")
    else:
        ...
        # FIX BUG 2: df_m['estado'] en lugar de df_m.get('estado')
        df_abiertos = df_m[df_m['estado'] == 'Abierto'] if not df_m.empty else pd.DataFrame()
        if not df_abiertos.empty:
            st.error(f"ALERTA: {len(df_abiertos)} Falla(s) en Curso (Tickets Abiertos).")
            cols_ok = [c for c in ['zona_completa','equipo_afectado','inicio_incidente','causa_raiz','descripcion'] if c in df_abiertos.columns]
            st.dataframe(df_abiertos[cols_ok].rename(columns={'zona_completa':'Nodo','equipo_afectado':'Equipo','inicio_incidente':'Hora Caida','causa_raiz':'Diagnostico'}), hide_index=True, use_container_width=True)
            st.divider()

        st.markdown("### Estado General de la Red")
        st.caption("Metricas globales calculadas excluyendo Mantenimientos Programados.")
        ...
        c4.metric("Impacto Acumulado",   f"{kpis['global']['db']/24:.2f} dias",    delta=_delta('global','db',divisor=24,fmt="{:+.2f}",suffix="d"), delta_color="inverse")
        c5.metric("Falla Mayor (Pico)",  f"{kpis['global']['mh']:.1f} horas",      delta=_delta('global','mh',fmt="{:+.1f}",suffix="h"),   delta_color="inverse")
        st.divider()

        st.markdown("### SLA por Zona (Disponibilidad Geografica)")
        ...
        fig_sla = px.bar(df_sla_geo, x="SLA (%)", y="Zona", orientation='h', text_auto='.3f', color="SLA (%)",
            color_continuous_scale=["#ff2b2b","#ff9f43","#29b09d"], title="Ranking de Estabilidad Geografica")
        fig_sla.update_layout(xaxis=dict(range=[max(0, df_sla_geo["SLA (%)"].min()-2), 100.5]),
            margin=dict(l=0,r=0,t=40,b=0), paper_bgcolor="rgba(0,0,0,0)", height=350)
        st.plotly_chart(fig_sla, use_container_width=True)
        st.divider()

        st.markdown("### Impacto Directo a Clientes (Datos Exactos)")
        st.info("Inteligencia de Datos: Los Clientes Afectados se calculan con el historico de la CMDB Autonoma.")
        if kpis['t1']['fallas'] == 0:
            st.success("Sin incidentes masivos cerrados con afectacion a clientes en este periodo.")
        else:
            t1a, t1b, t1c, t1d = st.columns(4)
            t1a.metric("Fallas Completas",       kpis['t1']['fallas'],              delta=_delta('t1','fallas',fmt="{:+.0f}"),            delta_color="inverse")
            t1b.metric("MTTR Real (Resolucion)", f"{kpis['t1']['mttr']:.2f} horas", delta=_delta('t1','mttr',fmt="{:+.2f}",suffix="h"),  delta_color="inverse")
            t1c.metric("ACD Real (Afectacion)",  f"{kpis['t1']['acd']:.2f} horas",  delta=_delta('t1','acd',fmt="{:+.2f}",suffix="h"),   delta_color="inverse")
            t1d.metric("Clientes Afectados",     f"{kpis['t1']['clientes']:,}",     delta=_delta('t1','clientes',fmt="{:+.0f}"),          delta_color="inverse")
        st.divider()
    ...
    with tabs[t_idx]:
        ...
        with cf:
            with st.container(border=True):
                ...
                sz    = st.text_input("Sub-zona (Ej. Colonia Escalon)", value="General" if ag else "", key=f"sz_{fk}", disabled=ag)
                ...
                eq_f  = st.selectbox("Equipo",     equipos_form,            key=f"e_{fk}")
                ...
                if z_f == "San Salvador (Central)":
                    ...
                    st.warning("Centro de Datos: Equipos Core. No aplica conteo de ONTs.")
                    ...
                    cl_f    = st.number_input(f"Clientes Afectados (sugerencia historica: {d_cl})", min_value=0, value=d_cl, step=1, key=f"cl_{fk}")
                ...
                with ct1:
                    ...
                    else: hi_val = None; st.info("Sin hora exacta -> Evento Incompleto.")
                with ct2:
                    if es_abierto:
                        ff = fi; hf_val = None; hf_on = False
                        st.warning("El ticket se guardara como Falla en Curso.")
                    else:
                        ...
                        else: hf_val = None; st.info("Sin hora exacta -> Evento Incompleto.")
                dur = 0.0; conocimiento = "Parcial"
                if hi_on and hf_on and (not es_abierto):
                    dt_ini = datetime.combine(fi, hi_val); dt_fin = datetime.combine(ff, hf_val)
                    if dt_fin > dt_ini:
                        dur = max(0.01, round((dt_fin - dt_ini).total_seconds() / 3600, 2)); conocimiento = "Total"
                st.write("")
                cr_f   = st.selectbox("Causa Raiz", causas_form, key=f"cr_{fk}")
                desc_f = st.text_area("Descripcion del Evento", key=f"desc_{fk}")
                    ...
                    if not err:
                        with st.spinner("Guardando..."):
                            try:
                                ...
                                with engine.begin() as conn:
                                    conn.execute(text("""INSERT INTO incidents (zona,subzona,afectacion_general,servicio,categoria,equipo_afectado,estado,inicio_incidente,fin_incidente,clientes_afectados,impacto_porcentaje,causa_raiz,descripcion,duracion_horas,conocimiento_tiempos) VALUES (:z,:sz,:ag,:s,:c,:e,:est,:idi,:idf,:cl,:imp,:cr,:d,:dur,:con)"""),
                                    ...
                                    if z_f != "San Salvador (Central)":
                                        conn.execute(text("INSERT INTO cmdb_nodos (zona,equipo,clientes,fuente) VALUES (:z,:e,:cl,'Manual') ON CONFLICT (zona,equipo) DO UPDATE SET clientes=EXCLUDED.clientes WHERE EXCLUDED.clientes>cmdb_nodos.clientes"), {"z":z_f,"e":eq_f,"cl":cl_f})
                                log_audit("INSERT", f"Falla ({estado_db}) en {z_f}")
                                load_data_rango.clear(); get_all_cmdb_nodos.clear()
                                st.session_state['form_reset'] = st.session_state.get('form_reset',0) + 1
                                st.session_state.flash_msg = "Registro guardado exitosamente."; st.session_state.flash_type = "success"
                                st.rerun()
                            except Exception as e:
                                st.session_state.flash_msg = f"Error de BD: {e}"; st.session_state.flash_type = "error"; st.rerun()
        with ccx:
            st.markdown("#### Registros Recientes")
            if not df_m.empty:
                for _, r in df_m.sort_values('id', ascending=False).head(6).iterrows():
                    with st.container(border=True):
                        ico = "ABIERTO" if r.get('estado') == 'Abierto' else ("INT" if r.get('categoria') == CAT_INTERNA else "RED")
                        st.markdown(f"**[{ico}] {r.get('zona_completa')}**")
                        st.caption(f"{str(r.get('causa_raiz',''))[:30]}... | Clientes: {r.get('clientes_afectados',0)}")
...
if role in ('admin', 'auditor'):
    with tabs[t_idx]:
        st.markdown("### Historial, Auditoria y Edicion")
        df_abiertos = df_m[df_m['estado'] == 'Abierto'] if not df_m.empty else pd.DataFrame()
        if not df_abiertos.empty:
            st.warning(f"Tienes {len(df_abiertos)} falla(s) en curso. Podes cerrarlas aqui:")
            with st.expander("Cerrar Falla en Curso", expanded=True):
                with st.form("form_cerrar_ticket"):
                    def _fmt_inicio(ts):
                        try: return ts.strftime('%d/%m/%Y %H:%M') if pd.notnull(ts) else 'Sin hora'
                        except: return 'Sin hora'
                    t_opts = {r['id']: f"ID:{r['id']} | {r.get('zona_completa')} | Inicio:{_fmt_inicio(r.get('inicio_incidente'))}" for _, r in df_abiertos.iterrows()}
                    ...
                    f_fin  = c_fc1.date_input("Fecha de Restablecimiento")
                    ...
                    if st.form_submit_button("Cerrar Ticket", type="primary"):
                        ...
                        if pd.isnull(ini_dt):
                            st.error("Este ticket no tiene hora de inicio. Editalo manualmente.")
                        elif fin_dt_val < ini_dt:
                            st.error("La fecha/hora de cierre no puede ser menor al inicio.")
                        else:
                            ...
                            st.session_state.flash_msg = "Ticket cerrado correctamente."; st.session_state.flash_type = "success"; st.rerun()
        st.divider()
        st.markdown("#### Edicion Avanzada")
        st.info("Para Editar: doble clic en la celda. Para Eliminar: marca la casilla y presiona Eliminar.")
        papelera = st.toggle("Explorar Papelera de Reciclaje")
        if papelera: st.warning("Ves registros eliminados. Podes restaurarlos o borrarlos definitivamente.")
        ...
        if df_audit.empty:
            ...
            pg      = c_pg.number_input("Pagina", 1, tot_p, 1, key="p_bd")
            ...
            ed_df = st.data_editor(df_page.drop(columns=drop_cols, errors='ignore'),
                column_config={"Sel": st.column_config.CheckboxColumn("Sel", default=False), "id": None,
                ...
                use_container_width=True, hide_index=True, key="editor_incidentes_v10")
            ...
            if not f_sel.empty:
                if papelera:
                    if cb1.button("Restaurar Seleccionados", type="primary", use_container_width=True):
                        with engine.begin() as conn:
                            for rid in f_sel['id']: conn.execute(text("UPDATE incidents SET deleted_at=NULL WHERE id=:id"), {"id": int(rid)})
                        log_audit("RESTORE", f"{len(f_sel)} registros."); load_data_rango.clear()
                        st.session_state.flash_msg = "Registros restaurados."; st.session_state.flash_type = "success"; st.rerun()
                    if cb2.button("Destruir Definitivamente", type="secondary", use_container_width=True):
                        with engine.begin() as conn:
                            for rid in f_sel['id']: conn.execute(text("DELETE FROM incidents WHERE id=:id"), {"id": int(rid)})
                        log_audit("HARD DELETE", f"{len(f_sel)} registros borrados."); load_data_rango.clear()
                        st.session_state.flash_msg = "Registros destruidos."; st.session_state.flash_type = "success"; st.rerun()
                else:
                    if cb1.button("Eliminar Seleccionados", type="primary", use_container_width=True):
                        with engine.begin() as conn:
                            for rid in f_sel['id']: conn.execute(text("UPDATE incidents SET deleted_at=CURRENT_TIMESTAMP WHERE id=:id"), {"id": int(rid)})
                        log_audit("DELETE (SOFT)", f"{len(f_sel)} registros."); load_data_rango.clear()
                        st.session_state.flash_msg = "Registros en Papelera."; st.session_state.flash_type = "success"; st.rerun()
            if h_cam and (not papelera) and cb2.button("Guardar Ediciones Manuales", type="primary", use_container_width=True):
                ...
                for i, r in ed_df.iterrows():
                    if not strip_tz(ref_df.iloc[i]).equals(strip_tz(r.drop('Sel', errors='ignore'))):
                        if pd.isnull(r.get('inicio_incidente')): fechas_validas = False; break
                if not fechas_validas:
                    st.session_state.flash_msg = "Error: La Fecha de Inicio es obligatoria."; st.session_state.flash_type = "error"; st.rerun()
                else:
                    with engine.begin() as conn:
                        for i, r in ed_df.iterrows():
                            if not strip_tz(ref_df.iloc[i]).equals(strip_tz(r.drop('Sel', errors='ignore'))):
                                try:
                                    ...
                                    if pd.isnull(r.get('fin_incidente')): fin_dt_sql=None; dur_u=0.0; con_u="Parcial"; est_u="Abierto"
                                    else:
                                        fin_dt=pd.to_datetime(r.get('fin_incidente')); fin_dt_sql=fin_dt
                                        dur_u=max(0.01, round((fin_dt-ini_dt).total_seconds()/3600,2)); con_u="Total"; est_u="Cerrado"
                                except: fin_dt_sql=None; dur_u=0.0; con_u="Parcial"; est_u="Abierto"
                                try:
                                    cl_val = r.get('clientes_afectados', 0)
                                    cl_val = int(float(cl_val)) if pd.notnull(cl_val) and str(cl_val).strip()!='' else 0
                                except: cl_val = 0
                                conn.execute(text("UPDATE incidents SET zona=:z,subzona=:sz,afectacion_general=:ag,servicio=:s,categoria=:c,equipo_afectado=:e,estado=:est,inicio_incidente=:idi,fin_incidente=:idf,clientes_afectados=:cl,causa_raiz=:cr,descripcion=:d,duracion_horas=:dur,conocimiento_tiempos=:con WHERE id=:id"),
                                    {"z":str(r.get('zona','')),"sz":str(r.get('subzona','')),"ag":bool(r.get('afectacion_general',True)),"s":str(r.get('servicio','')),"c":str(r.get('categoria','')),"e":str(r.get('equipo_afectado','')),"est":est_u,"idi":ini_dt,"idf":fin_dt_sql,"cl":cl_val,"cr":str(r.get('causa_raiz','')),"d":str(r.get('descripcion','')),"dur":dur_u,"con":con_u,"id":int(r.get('id'))})
                    log_audit("UPDATE","Edicion masiva."); load_data_rango.clear()
                    st.session_state.flash_msg="Cambios guardados."; st.session_state.flash_type="success"; st.rerun()
            st.divider()
            st.download_button("Descargar CSV", df_d.drop(columns=drop_cols,errors='ignore').to_csv(index=False).encode(), f"NOC_{fecha_ini}_{fecha_fin}.csv","text/csv",use_container_width=True)
    t_idx += 1
...
if role == 'admin' and len(tabs) > t_idx:
    with tabs[t_idx]:
        ...
        t_zonas, t_equipos, t_causas, t_usuarios = st.tabs(["Zonas","Equipos","Causas","Usuarios y Accesos"])

        with t_zonas:
            st.markdown("#### Gestion de Zonas / Nodos")
            for z_nombre, z_lat, z_lon in get_zonas():
                c_zn, c_zdel = st.columns([5,1])
                c_zn.text(f"{z_nombre}  (Lat:{z_lat:.4f}, Lon:{z_lon:.4f})")
                if c_zdel.button("Eliminar", key=f"del_z_{z_nombre}"):
                    try:
                        with engine.begin() as conn: conn.execute(text("DELETE FROM cat_zonas WHERE nombre=:n"), {"n":z_nombre})
                        clear_catalog_cache(); st.session_state.flash_msg="Zona eliminada."; st.rerun()
                    except: st.session_state.flash_msg="Error: zona en uso."; st.session_state.flash_type="error"; st.rerun()
            st.divider()
            with st.form("form_add_zona", clear_on_submit=True):
                ...
                if st.form_submit_button("Agregar Zona") and nz:
                    ...
                    except: st.toast("Error: Zona duplicada.")

        with t_equipos:
            st.markdown("#### Gestion de Equipos")
            for eq in get_cat("cat_equipos"):
                c_en, c_edel = st.columns([5,1])
                c_en.text(eq)
                if c_edel.button("Eliminar", key=f"del_eq_{eq}"):
                    try:
                        with engine.begin() as conn: conn.execute(text("DELETE FROM cat_equipos WHERE nombre=:n"), {"n":eq})
                        clear_catalog_cache(); st.session_state.flash_msg="Equipo eliminado."; st.rerun()
                    except: st.session_state.flash_msg="Error: equipo en uso."; st.session_state.flash_type="error"; st.rerun()
            st.divider()
            with st.form("form_add_eq", clear_on_submit=True):
                st.markdown("**Agregar Equipo**")
                ne = st.text_input("Nombre del dispositivo")
                if st.form_submit_button("Agregar") and ne:
                    try:
                    ...
                    except: st.toast("Error: Equipo duplicado.")

        with t_causas:
            st.markdown("#### Gestion de Causas Raiz")
            for causa, ext in get_causas_con_flag().items():
                c_cn, c_ct, c_cdel = st.columns([4,2,1])
                c_cn.text(causa)
                c_ct.caption("Externa" if ext else "Interna")
                if c_cdel.button("Eliminar", key=f"del_ca_{causa}"):
                    try:
                        with engine.begin() as conn: conn.execute(text("DELETE FROM cat_causas WHERE nombre=:n"), {"n":causa})
                        clear_catalog_cache(); st.session_state.flash_msg="Causa eliminada."; st.rerun()
                    except: st.session_state.flash_msg="Error: causa en uso."; st.session_state.flash_type="error"; st.rerun()
            st.divider()
            with st.form("form_add_causa", clear_on_submit=True):
                st.markdown("**Agregar Causa**")
                nc  = st.text_input("Descripcion")
                nce = st.checkbox("Es factor Externo (Terceros, Clima, etc)?")
                if st.form_submit_button("Agregar") and nc:
                    try:
                    ...
                    except: st.toast("Error: Causa duplicada.")
        ...
        with t_usuarios:
            ...
            with cu:
                with st.form("form_u", clear_on_submit=True):
                    ...
                    np_u = st.text_input("Contrasena", type="password")
                    st.caption("Min. 8 chars, 1 mayuscula, 1 minuscula, 1 numero, 1 especial (@$!%*?&)")
                    ...
                    if st.form_submit_button("Crear Cuenta") and nu and np_u:
                        if not es_password_segura(np_u):
                            st.session_state.flash_msg = "Contrasena no cumple requisitos de seguridad."; st.session_state.flash_type="error"; st.rerun()
                        else:
                            try:
                                with engine.begin() as conn: conn.execute(text("INSERT INTO users (username,password_hash,role) VALUES (:u,:h,:r)"), {"u":nu,"h":hash_pw(np_u),"r":nrl})
                                st.session_state.flash_msg="Usuario creado."; st.rerun()
                            except Exception as e:
                                st.toast("Error: usuario duplicado." if "duplicate" in str(e).lower() or "unique" in str(e).lower() else f"Error: {e}")

                with st.form("form_reset_pw", clear_on_submit=True):
                    st.markdown("**Restablecer Contrasena**")
                    u_reset = st.text_input("Usuario exacto")
                    p_reset = st.text_input("Nueva Contrasena", type="password")
                    if st.form_submit_button("Restablecer") and u_reset and p_reset:
                        if not es_password_segura(p_reset):
                            st.session_state.flash_msg="La nueva contrasena no cumple los requisitos."; st.session_state.flash_type="error"; st.rerun()
                        else:
                            try:
                                with engine.begin() as conn:
                                    ...
                                    if res.rowcount > 0: st.session_state.flash_msg="Contrasena actualizada."; st.rerun()
                                    else: st.toast("Usuario no encontrado.")
            ...
            with clg:
                try:
                    ...
                    ed_usrs = st.data_editor(df_usrs,
                        column_config={"Sel":st.column_config.CheckboxColumn("Sel",default=False),"id":None,
                            "username":"Usuario","role":"Rol","is_banned":"Baneado","failed_attempts":"Intentos"},
                        use_container_width=True, hide_index=True, key="editor_usuarios_v10")
                    filas_del   = ed_usrs[ed_usrs["Sel"] == True]
                    hay_cambios = not (df_usrs.drop(columns=['Sel'],errors='ignore').reset_index(drop=True).equals(ed_usrs.drop(columns=['Sel'],errors='ignore').reset_index(drop=True)))
                    u1c, u2c = st.columns(2)
                    if not filas_del.empty and u1c.button("Eliminar Usuario", use_container_width=True):
                        if "Admin" in filas_del['username'].values: st.error("No se puede eliminar el Admin raiz.")
                        elif st.session_state.get('username') in filas_del['username'].values:
                            st.session_state.flash_msg="No podes eliminar tu propia cuenta."; st.session_state.flash_type="error"; st.rerun()
                        else:
                            with engine.begin() as conn:
                                for rid in filas_del['id']: conn.execute(text("DELETE FROM users WHERE id=:id"), {"id":int(rid)})
                            st.session_state.flash_msg="Usuario eliminado."; st.rerun()
                            ...
                            for i, er in ed_usrs.iterrows():
                                ...
                                if not orig.equals(er.drop('Sel',errors='ignore')):
                                    conn.execute(text("UPDATE users SET role=:r,is_banned=:b,failed_attempts=:f WHERE id=:id"),
                                        {"r":str(er.get('role','viewer')),"b":bool(er.get('is_banned',False)),"f":int(er.get('failed_attempts',0)),"id":int(er.get('id'))})
            ...
            try:
                ...
                p_log = st.number_input("Pagina de log", 1, t_lp, 1)
