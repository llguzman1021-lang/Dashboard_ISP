...
[data-testid="stMetricLabel"] { color: #a5a8b5 !important; font-size: 14px !important; }
...
def init_db():
    with engine.begin() as conn:
        ...
        """))
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS cat_causas (
                id SERIAL PRIMARY KEY,
                nombre VARCHAR(150) UNIQUE NOT NULL,
        ...
        # Seed catálogos solo si están vacíos
        cnt_z = conn.execute(text("SELECT count(*) FROM cat_zonas")).scalar()
        if cnt_z == 0:
            for nombre, lat, lon in DEFAULT_ZONAS:
                conn.execute(text("INSERT INTO cat_zonas (nombre, lat, lon) VALUES (:n, :lat, :lon) ON CONFLICT DO NOTHING"),
                             {"n": nombre, "lat": lat, "lon": lon})

        cnt_e = conn.execute(text("SELECT count(*) FROM cat_equipos")).scalar()
        if cnt_e == 0:
            for eq in DEFAULT_EQUIPOS:
                conn.execute(text("INSERT INTO cat_equipos (nombre) VALUES (:n) ON CONFLICT DO NOTHING"), {"n": eq})

        cnt_c = conn.execute(text("SELECT count(*) FROM cat_causas")).scalar()
        if cnt_c == 0:
            for nombre, ext in DEFAULT_CAUSAS:
                conn.execute(text("INSERT INTO cat_causas (nombre, es_externa) VALUES (:n, :e) ON CONFLICT DO NOTHING"),
                             {"n": nombre, "e": ext})

        cnt_s = conn.execute(text("SELECT count(*) FROM cat_servicios")).scalar()
        if cnt_s == 0:
            for srv in DEFAULT_SERVICIOS:
                conn.execute(text("INSERT INTO cat_servicios (nombre) VALUES (:n) ON CONFLICT DO NOTHING"), {"n": srv})
        ...
        # Admin por defecto
        cnt_adm = conn.execute(text("SELECT count(*) FROM users WHERE username='Admin'")).scalar()
        if cnt_adm == 0:
            conn.execute(text("""
                INSERT INTO users (username,password_hash,role,pregunta,respuesta)
                VALUES ('Admin',:h,'admin','¿Color favorito?','azul')
            """), {"h": hash_pw("Areakde5")})
...
# ─── Helpers de catálogo (siempre frescos) ───
def get_zonas():
    try:
        ...
        return [(n, lat, lon) for n, lat, lon in DEFAULT_ZONAS]

def get_cat(tabla):
    try:
        with engine.connect() as c:
            rows = c.execute(text(f"SELECT nombre FROM {tabla} ORDER BY nombre")).fetchall()
...
def get_causas_con_flag():
    try:
        with engine.connect() as c:
            rows = c.execute(text("SELECT nombre, es_externa FROM cat_causas ORDER BY nombre")).fetchall()
...
def load_data_rango(fecha_ini: date, fecha_fin: date, include_deleted=False,
                    zona_filtro="Todas", serv_filtro="Todos", seg_filtro="Todos"):
...
def _merge_intervals(intervals):
    """Fusiona intervalos solapados. Recibe lista de [start, end] datetime."""
...
def calc_kpis(df, fecha_ini: date, fecha_fin: date):
    ...
    base = {
        "sla": 100.0, "mttr": 0.0, "acd": 0.0, "riesgo_usd": 0.0,
    ...
    # Incompletos: sin tiempo (conocimiento_tiempos != 'Total')
    ...
    if not df_t1.empty:
        ...
        base['riesgo_usd'] = float((df_t1['duracion_horas'] * df_t1['clientes_afectados']).sum()
                                   * ARPU_USD / HOURS_MONTH)

    # SLA — usar todos los que tienen tiempo exacto (t1 + sin_clientes)
...
def generar_pdf(label_periodo, kpis, df):
    ...
    kpi_rows = [
        ...
        ['MTTR Real',                     f"{kpis['mttr']:.2f} hrs",'Tiempo promedio de resolución (clientes > 0)'],
        ['ACD (Afectación por Cliente)',  f"{kpis['acd']:.2f} hrs",'Percepción real del usuario promedio'],
        ['Riesgo Financiero (Est.)',      f"${kpis['riesgo_usd']:,.0f}",'ARPU × horas × clientes / 720'],
        ['Clientes Impactados',           f"{kpis['clientes']:,}", 'Total acumulado de usuarios afectados'],
        ['MTBF (Estabilidad)',            f"{kpis['mtbf']:.1f} hrs",'Tiempo entre fallas — datos exactos'],
...
# =====================================================================
with st.sidebar:
    st.caption(f"👤 **{st.session_state.username}** ({st.session_state.role})  |  NOC v22.0")
    ...
    rango = st.date_input(
        "📅 Rango de Análisis",
        value=(primer_dia, today),
        min_value=date(2020, 1, 1),
        max_value=date(today.year + 1, 12, 31),
        format="YYYY-MM-DD",
    )
    if isinstance(rango, (list, tuple)) and len(rango) == 2:
        fecha_ini, fecha_fin = rango[0], rango[1]
    else:
        fecha_ini = fecha_fin = today

    label_periodo = f"{fecha_ini.strftime('%d/%m/%Y')} → {fecha_fin.strftime('%d/%m/%Y')}"
    ...
    if not df_m.empty:
        ...
        st.download_button(
            ...
            data=pdf_data,
            file_name=f"Reporte_NOC_{fecha_ini}_{fecha_fin}.pdf",
...
if role == 'admin':
    ...
    tab_labels = ["📊 Dashboard", "📝 Registrar Evento"]
else:  # viewer
...
with tabs[0]:
    ...
    if df_m.empty:
        ...
        # ── Métricas Operativas ──
        st.markdown("### 🌟 Métricas Operativas Principales")
        st.caption("*Basadas exclusivamente en registros con tiempos exactos y clientes > 0*")

        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("SLA Global",        f"{kpis['sla']:.3f}%",
                  help="Disponibilidad con fusión de intervalos solapados. Sin doble descuento.")
        c2.metric("MTTR Real",         f"{kpis['mttr']:.2f} hrs",
                  help="Tiempo Medio de Resolución — fallas con clientes afectados.")
        c3.metric("ACD Real",          f"{kpis['acd']:.2f} hrs",
                  help="Afectación promedio percibida por el usuario.")
        c4.metric("Riesgo Financiero", f"${kpis['riesgo_usd']:,.0f}",
                  help=f"Estimado: ARPU ${ARPU_USD}/mes × ACD × clientes / {HOURS_MONTH}h")
        c5.metric("Clientes Afect.",   f"{kpis['clientes']:,}",
                  help="Total acumulado de usuarios con interrupción confirmada.")

        st.write("")
        c6, c7, c8 = st.columns(3)
        c6.metric("MTBF (Estabilidad)", f"{kpis['mtbf']:.1f} hrs",
                  help="Tiempo medio entre fallas — calculado con intervalos exactos.")
        c7.metric("Falla Mayor (Pico)", f"{kpis['mh']:.1f} hrs",
                  help="Duración máxima de un solo evento en el periodo.")
        c8.metric("Fallas P1 Críticas", kpis['p1'],
                  help=">12h o >1000 clientes.")

        st.divider()

        # ── Panel de Salud de Datos ──
        st.markdown("### ⚠️ Salud de Datos / Pendientes")
        st.caption("*Eventos excluidos de métricas — requieren actualización en BD*")

        p1c, p2c, p3c = st.columns(3)
        with p1c:
            with st.container(border=True):
                v = kpis['pendiente_sin_cierre']
                color = "🔴" if v > 0 else "🟢"
                st.metric(f"{color} Sin tiempo de cierre", v,
                          help="Registros marcados como 'Parcial'. El SLA no puede calcularse sin hora de fin.")
        with p2c:
            with st.container(border=True):
                v = kpis['pendiente_sin_clientes']
                color = "🟡" if v > 0 else "🟢"
                st.metric(f"{color} Sin clientes definidos", v,
                          help="Fallas de red aislada con tiempo exacto pero sin clientes (0). Se excluyen de MTTR/ACD.")
        with p3c:
            with st.container(border=True):
                v = kpis['internas']
                color = "🔵" if v > 0 else "🟢"
                st.metric(f"{color} Incidencias internas", v,
                          help="Fallas internas / mantenimiento programado. No impactan SLA externo.")
        ...
        with c_map:
            df_map = df_m.copy()
            df_map['lat'] = df_map['zona'].map(
                lambda x: zonas_coords.get(x, (13.6929, -89.2182))[0])
            df_map['lon'] = df_map['zona'].map(
                lambda x: zonas_coords.get(x, (13.6929, -89.2182))[1])
            agg = (df_map.groupby(['zona_completa','lat','lon'])
                   .agg(Horas=('duracion_horas','sum'),
                        Clientes=('clientes_afectados','sum'))
                   .reset_index())
            agg['Clientes_sz'] = agg['Clientes'].clip(lower=1)  # evitar size=0 en scatter
            fig_m = px.scatter_mapbox(
                agg, lat="lat", lon="lon",
                hover_name="zona_completa",
                size="Clientes_sz", color="Horas",
                color_continuous_scale="Inferno",
                zoom=8.5, mapbox_style="carto-darkmatter",
                labels={"Clientes_sz": "Clientes"},
            )
            fig_m.update_layout(margin=dict(l=0,r=0,t=0,b=0), height=370)
        ...
        with c_heat:
            ...
            df_t2      = df_m.copy()
            df_t2['Día']  = pd.Categorical(df_t2['inicio_incidente'].dt.dayofweek.map(dias_map),
            ...
            fig_hm = px.density_heatmap(df_hm, x='Hora', y='Día', z='Fallas',
        ...
        with c_bar:
            ...
            fig_b.update_layout(margin=dict(l=0,r=0,t=20,b=0),
                                 paper_bgcolor="rgba(0,0,0,0)", height=330,
                                 xaxis_title="", yaxis_title="")
            st.plotly_chart(fig_b, use_container_width=True)

# ─────────────────────────────────────────────
# TAB 1 — REGISTRAR EVENTO (admin + tech)
# ─────────────────────────────────────────────
if role in ('admin', 'tech') and len(tabs) > 1:
    with tabs[1]:
        st.title("📝 Registrar Evento Operativo")
        fk = st.session_state.form_reset

        # CMDB para autocompletar clientes
        try:
        ...
        with cf:
            with st.container(border=True):
                ...
                if cat_f == "Cliente Corporativo":
                    ...
                    cl_f = st.number_input("👤 Clientes Afectados (0 si no se sabe)",
...
# ─────────────────────────────────────────────
# TAB 2 — HISTORIAL Y EDICIÓN (solo admin)
# ─────────────────────────────────────────────
    ...
    with tabs[2]:
        ...
        if df_audit.empty:
            ...
            drop_cols = [c for c in
                         ['deleted_at','Severidad','zona_completa','data_quality_flag','es_externa']
            ...
            ref_df = (df_page.drop(columns=drop_cols + (['Sel'] if 'Sel' in drop_cols else []),
                                   errors='ignore')
                ...
                if papelera:
                    ...
                    if cb1.button("🗑️ Eliminar Seleccionados", type="primary", use_container_width=True):
                        with engine.begin() as conn:
                            for rid in f_sel['id']:
                                conn.execute(text("""
                                    UPDATE incidents SET deleted_at=CURRENT_TIMESTAMP WHERE id=:id
                                """), {"id": int(rid)})
            ...
            st.download_button(
                ...
                f"NOC_{fecha_ini}_{fecha_fin}.csv", "text/csv",
                use_container_width=True,
            )
...
if role == 'admin' and len(tabs) > 3:
    with tabs[3]:
        ...
        st.caption("Gestión de catálogos y usuarios. Los cambios se reflejan inmediatamente en el formulario de registro.")
        ...
        with cfg1:
            with st.container(border=True):
                st.markdown("#### 🗺️ Zonas / Nodos")
                zonas_bd = get_zonas()
                for z_nombre, z_lat, z_lon in zonas_bd:
                    c_zn, c_zdel = st.columns([3, 1])
                    c_zn.text(f"{z_nombre}  ({z_lat:.4f}, {z_lon:.4f})")
                ...
                with st.form("form_add_zona", clear_on_submit=True):
                    st.markdown("**Agregar Zona**")
                    nz    = st.text_input("Nombre")
                    ...
                    if st.form_submit_button("➕ Agregar") and nz:
                        try:
                            with engine.begin() as conn:
                                conn.execute(text("INSERT INTO cat_zonas (nombre,lat,lon) VALUES (:n,:la,:lo)"),
                                             {"n": nz, "la": nlat, "lo": nlon})
                            st.rerun()
                        except:
                            st.error("Zona duplicada o error.")
                ...
                with st.form("form_add_eq", clear_on_submit=True):
                    ...
                    if st.form_submit_button("➕ Agregar") and ne:
                        try:
                            ...
                            st.error("Equipo duplicado o error.")

        st.write("")
        cfg3, cfg4 = st.columns(2, gap="large")

        # ── CAUSAS ──
        with cfg3:
            with st.container(border=True):
                st.markdown("#### 🛠️ Causas Raíz")
                causas_bd = get_causas_con_flag()
                for causa, ext in causas_bd.items():
                    c_cn, c_ct, c_cdel = st.columns([3,1,1])
                    c_cn.text(causa[:35])
                ...
                with st.form("form_add_causa", clear_on_submit=True):
                    ...
                    nc  = st.text_input("Nombre de la causa")
                    nce = st.checkbox("¿Es causa externa / fuerza mayor?")
                    if st.form_submit_button("➕ Agregar") and nc:
                        try:
                            with engine.begin() as conn:
                                conn.execute(text("INSERT INTO cat_causas (nombre,es_externa) VALUES (:n,:e)"),
                                             {"n": nc, "e": nce})
                            st.rerun()
                        except:
                            st.error("Causa duplicada o error.")
        ...
        with cfg4:
            with st.container(border=True):
                ...
                with st.form("form_u", clear_on_submit=True):
                    ...
                    nrl   = st.selectbox("Rol", ["viewer","tech","admin"])
                    ...
                    if st.form_submit_button("➕ Crear Usuario") and nu and np_u:
                        try:
                            ...
                            st.toast("❌ Nombre duplicado.", icon="❌")
                ...
                try:
                    ...
                    hay_cambios = not (df_usrs.drop(columns=['Sel'])
                                       .reset_index(drop=True)
                    ...
                    if hay_cambios and u2c.button("💾 Guardar Permisos", use_container_width=True):
                        with engine.begin() as conn:
        ...
        try:
            with engine.connect() as conn:
                logs = pd.read_sql(text("""
                    SELECT timestamp AS Fecha, username AS Usuario,
                           action AS Accion, details AS Detalles
