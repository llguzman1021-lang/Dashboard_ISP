...
def init_db():
    with engine.begin() as conn:
        ...
        if conn.execute(text("SELECT count(*) FROM cat_zonas")).scalar() == 0:
            for nombre, lat, lon in DEFAULT_ZONAS: conn.execute(text("INSERT INTO cat_zonas (nombre, lat, lon) VALUES (:n, :lat, :lon) ON CONFLICT DO NOTHING"), {"n": nombre, "lat": lat, "lon": lon})
        ...
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS incidents (
                id SERIAL PRIMARY KEY, zona VARCHAR(150) NOT NULL, subzona VARCHAR(150), afectacion_general BOOLEAN DEFAULT TRUE, servicio VARCHAR(100), categoria VARCHAR(100), equipo_afectado VARCHAR(100), inicio_incidente TIMESTAMPTZ NOT NULL, fin_incidente TIMESTAMPTZ NOT NULL, clientes_afectados INT DEFAULT 0, causa_raiz VARCHAR(150), descripcion TEXT, duracion_horas FLOAT, conocimiento_tiempos VARCHAR(50) DEFAULT 'Total', deleted_at TIMESTAMPTZ DEFAULT NULL
            )
        """))
...
def calc_kpis(df, fecha_ini: date, fecha_fin: date):
    ...
    base["global"]["db"] = float(df_exact['duracion_horas'].sum()) if not df_exact.empty else 0.0
    base["global"]["mh"] = float(df_exact['duracion_horas'].max()) if not df_exact.empty else 0.0
    ...
    up_hrs = h_tot - t_down
    base["global"]["mtbf"] = float(up_hrs / len(df_exact)) if len(df_exact) > 0 else float(h_tot)
...
def generar_pdf(label_periodo, kpis, df):
    ...
    def tbl_style(hdr_color):
        return TableStyle([('BACKGROUND',  (0,0), (-1,0),  hdr_color), ('TEXTCOLOR',   (0,0), (-1,0),  rl_colors.white), ('FONTNAME',    (0,0), (-1,0),  'Helvetica-Bold'), ('FONTSIZE',    (0,0), (-1,0),  9), ('ROWBACKGROUNDS',(0,1),(-1,-1),[RL_LIGHT, rl_colors.white]), ('FONTNAME',    (0,1), (-1,-1), 'Helvetica'), ('FONTSIZE',    (0,1), (-1,-1), 9), ('VALIGN',      (0,0), (-1,-1), 'MIDDLE'), ('GRID',        (0,0), (-1,-1), 0.4, RL_LGRAY), ('ROWHEIGHT',   (0,0), (-1,-1), 20), ('LEFTPADDING', (0,0), (-1,-1), 7), ('RIGHTPADDING',(0,0), (-1,-1), 7), ('TEXTCOLOR',   (1,1), (1,-1),  RL_ORANGE), ('FONTNAME',    (1,1), (1,-1),  'Helvetica-Bold'), ('ALIGN',       (1,0), (1,-1),  'CENTER')])
    ...
    story += [Paragraph("MULTINET", s_title), Paragraph("Reporte Ejecutivo · Network Operations Center", s_sub), HRFlowable(width="100%", thickness=2, color=RL_ORANGE, spaceAfter=10), Paragraph(f"<b>Periodo:</b> {label_periodo}", s_period), Paragraph(f"<b>Generado:</b> {now_str} (hora El Salvador)", s_body), Spacer(1, 0.5*cm)]
    ...
    kpi_rows = [
        ...
        ['MTTR Real',                     f"{kpis['t1']['mttr']:.2f} horas",'Tiempo promedio de resolución (clientes > 0)'],
        ['ACD Real (Afectación)',         f"{kpis['t1']['acd']:.2f} horas", 'Percepción real del usuario promedio'],
        ['Impacto Acumulado',             f"{(kpis['global']['db']/24):.2f} días",'Horas totales caídas convertidas a días'],
        ['Clientes Impactados',           f"{kpis['t1']['clientes']:,}", 'Total acumulado de usuarios afectados'],
        ['MTBF (Estabilidad)',            f"{kpis['global']['mtbf']:.1f} horas",'Tiempo entre fallas — datos exactos'],
        ['Fallas P1 Críticas',            f"{kpis['global']['p1']}",          '>12h duración o >1000 clientes'],
    ]
    t = Table(kpi_rows, colWidths=[5.5*cm, 3*cm, 8.5*cm]); t.setStyle(tbl_style(RL_BLUE)); story += [t, Spacer(1, 0.4*cm)]
    ...
    tp = Table(pend_rows, colWidths=[10*cm, 4*cm]); tp.setStyle(tbl_style(RL_TEAL)); story += [tp, Spacer(1, 0.4*cm)]

    if not df.empty:
        df_ext = df[df['categoria'] != CAT_INTERNA]; story.append(Paragraph("3. Zonas con Mayor Afectación", s_sec)); top_z = df_ext.groupby('zona_completa')['duracion_horas'].sum().nlargest(8).reset_index()
        if not top_z.empty:
            ...
            tz = Table(z_rows, colWidths=[9*cm, 4*cm, 4*cm]); tz.setStyle(tbl_style(RL_TEAL)); story += [tz, Spacer(1, 0.4*cm)]

    story += [Spacer(1, 1*cm), HRFlowable(width="100%", thickness=0.5, color=RL_LGRAY, spaceAfter=6), Paragraph(f"MULTINET NOC  ·  {now_str}  ·  Documento Confidencial", s_foot)]
...
if not st.session_state.logged_in:
    ...
    with col_c:
        ...
        with st.container(border=True):
            ...
            st.text_input("Usuario", key="log_u"); st.text_input("Contraseña", key="log_p", type="password"); st.write("")
            ...
            with st.expander("¿Olvidó su contraseña?"):
                ...
                if ru:
                    try:
                        ...
                        if ud2:
                            st.info(f"**Pregunta:** {ud2[0]}"); rr = st.text_input("Respuesta:", type="password")
                            if rr:
...
# =====================================================================
with st.sidebar:
    st.caption(f"👤 **{st.session_state.username}** ({st.session_state.role.capitalize()})  |  NOC v23.1")
    ...
    df_m = enriquecer(load_data_rango(fecha_ini, fecha_fin, include_deleted=False, zona_filtro=z_sel, serv_filtro=srv_sel, seg_filtro=seg_sel))

    # Cargar datos mes anterior (Para deltas comparativos)
    if m_idx == 1: p_m_idx = 12; p_a_sel = a_sel - 1
    else: p_m_idx = m_idx - 1; p_a_sel = a_sel
    ...
    df_prev = enriquecer(load_data_rango(p_fecha_ini, p_fecha_fin, include_deleted=False, zona_filtro=z_sel, serv_filtro=srv_sel, seg_filtro=seg_sel))
...
with tabs[0]:
    ...
    if df_m.empty:
        ...
        def get_delta(cat, key, format_str="{:+.2f}", suffix="", divisor=1):
        ...
        c1.metric("SLA Global", f"{kpis['global']['sla']:.3f}%", delta=get_delta('global', 'sla', "{:+.3f}", "%"), delta_color="normal", help="Porcentaje del tiempo operativo. No resta doble por fallas simultáneas.")
        c2.metric("Total Eventos Registrados", kpis['global']['total_fallas'], delta=get_delta('global', 'total_fallas', "{:+.0f}"), delta_color="inverse", help="Suma de todas las fallas externas registradas.")
        c3.metric("MTBF (Estabilidad)", f"{kpis['global']['mtbf']:.1f} horas", delta=get_delta('global', 'mtbf', "{:+.1f}", "horas"), delta_color="normal", help="Tiempo Medio Entre Fallas.")
        c4.metric("Impacto Acumulado", f"{(kpis['global']['db']/24):.2f} días", delta=get_delta('global', 'db', "{:+.2f}", "días", divisor=24), delta_color="inverse", help="Suma total de horas de caída de red convertidas a días.")
        c5.metric("Falla Mayor (Pico)", f"{kpis['global']['mh']:.1f} horas", delta=get_delta('global', 'mh', "{:+.1f}", "horas"), delta_color="inverse", help="Duración del evento de interrupción más largo.")
        ...
        if kpis['t1']['fallas'] == 0: st.info("ℹ️ No hay incidentes masivos con tiempos exactos en este periodo.")
            ...
            t1a.metric("Fallas Completas", kpis['t1']['fallas'], delta=get_delta('t1', 'fallas', "{:+.0f}"), delta_color="inverse", help="Eventos que poseen duración exacta y número real de clientes > 0.")
            t1b.metric("MTTR Real", f"{kpis['t1']['mttr']:.2f} horas", delta=get_delta('t1', 'mttr', "{:+.2f}", "horas"), delta_color="inverse", help="Tiempo Promedio de Resolución. Tiempo de arreglo de fallas masivas.")
            t1c.metric("ACD Real", f"{kpis['t1']['acd']:.2f} horas", delta=get_delta('t1', 'acd', "{:+.2f}", "horas"), delta_color="inverse", help="Afectación Promedio por Cliente. Sensación real de corte por usuario.")
            t1d.metric("Clientes Afectados", f"{kpis['t1']['clientes']:,}", delta=get_delta('t1', 'clientes', "{:+.0f}"), delta_color="inverse", help="Suma total de clientes que sufrieron cortes en este mes.")
        ...
        with col_t2:
            with st.container(border=True):
                v_t2 = kpis['t2']['fallas']
                st.markdown(f"#### {'🟡' if v_t2 > 0 else '🟢'} Fallas de Red Pura")
                st.metric("Eventos sin Clientes", v_t2, help="Tienen hora exacta, pero 0 clientes afectados. Afectan SLA pero no ACD.")
                st.metric("MTTR Infraestructura", f"{kpis['t2']['mttr']:.2f} horas", help="Resolución de fallas de red aisladas.")
                
        with col_t3:
            with st.container(border=True):
                v_t3 = kpis['t3']['fallas']
                st.markdown(f"#### {'🔴' if v_t3 > 0 else '🟢'} Eventos Incompletos")
                st.metric("Fallas Sin Tiempo", v_t3, help="Falta hora de inicio o cierre. Se asumen duración 0. Excluidas del SLA.")
                st.metric("Clientes Estimados", f"{kpis['t3']['clientes_est']:,}", help="Clientes posiblemente afectados por estas fallas.")
                
        with col_int:
            with st.container(border=True):
                v_int = kpis['int']['fallas']
                st.markdown(f"#### {'🔵' if v_int > 0 else '🟢'} Internas / Mantenimiento")
                st.metric("Eventos Internos", v_int, help="Problemas controlados o de oficinas que no impactan el servicio externo.")
                st.metric("MTTR Interno", f"{kpis['int']['mttr']:.2f} horas", help="Tiempo de resolución interno.")
        ...
        fig_m = px.scatter_mapbox(agg, lat="lat", lon="lon", hover_name="zona_completa", size="Clientes_sz", color="Horas", color_continuous_scale="Inferno", zoom=8.5, mapbox_style="carto-darkmatter", labels={"Clientes_sz": "Clientes"}, title="Impacto Geográfico")
        fig_m.update_layout(margin=dict(l=0,r=0,t=40,b=0), height=500)
        ...
        fig_hm = px.density_heatmap(df_hm, x='Hora', y='Día', z='Fallas', color_continuous_scale='Blues', nbinsx=24, title="Mapa de Calor (Concentración de Fallas por Horario)")
        fig_hm.update_layout(xaxis=dict(tickmode='linear', tick0=0, dtick=1), margin=dict(l=0,r=0,t=40,b=0), paper_bgcolor="rgba(0,0,0,0)", height=450)
        ...
        fig_p.update_layout(showlegend=True, legend=dict(orientation="h", yanchor="top", y=-0.1, xanchor="center", x=0.5), margin=dict(l=0,r=0,t=40,b=0), paper_bgcolor="rgba(0,0,0,0)", height=450)
        st.plotly_chart(fig_p, use_container_width=True)

        st.write("")
        dc = (df_m.groupby('causa_raiz').size().reset_index(name='Alertas').sort_values('Alertas', ascending=True).tail(10))
        ...
        fig_b.update_layout(margin=dict(l=0,r=0,t=40,b=0), paper_bgcolor="rgba(0,0,0,0)", height=450, xaxis_title="", yaxis_title="")
    ...
    with tabs[1]:
        ...
        with cf:
            with st.container(border=True):
                ...
                sz = st.text_input("📍 Sub-zona (Ej. San Antonio Masahuat)", value="General" if ag else "", key=f"sz_{fk}", disabled=ag)
                st.divider()
                ...
                if cat_f == "Cliente Corporativo":
                    ...
                    if not cmdb_df.empty:
                        ...
                        m = cmdb_df[(cmdb_df['zona'] == z_f) & (cmdb_df['subzona'] == sz_val) & (cmdb_df['equipo'] == eq_f)]
                        if not m.empty: d_cl = int(m['clientes'].iloc[0])
                    cl_f = st.number_input("👤 Clientes Afectados (0 si no se sabe)", min_value=0, value=d_cl, step=1, key=f"cl_{fk}")
            ...
            if not df_m.empty:
                for _, r in df_m.sort_values('id', ascending=False).head(6).iterrows():
                    with st.container(border=True):
                        ...
                        st.caption(f"{causa_str}… | ⏳ {r['duracion_horas']}h | 👥 {r['clientes_afectados']}")
...
if role in ('admin', 'auditor') and len(tabs) > 2:
    with tabs[2]:
        ...
        df_audit = enriquecer(load_data_rango(fecha_ini, fecha_fin, include_deleted=papelera, zona_filtro="Todas", serv_filtro="Todos", seg_filtro="Todos"))
